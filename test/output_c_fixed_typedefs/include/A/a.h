...
// This version is acting on mutable pointers.
// The reference to the parameter `object` might be preserved in the return value.
MR_A_API const MR_SeparateA *MR_SeparateA_StaticDowncastFrom_MR_CommonBaseAB(const MR_CommonBaseAB *object);

// Downcasts an instance of `MR::CommonBaseAB` to a derived class `MR::SeparateA`.
// This is a static downcast, it trusts the programmer that the target type is correct. Results in UB and returns an invalid pointer otherwise.
// The reference to the parameter `object` might be preserved in the return value.
MR_A_API MR_SeparateA *MR_SeparateA_MutableStaticDowncastFrom_MR_CommonBaseAB(MR_CommonBaseAB *object);

// Downcasts an instance of `MR::CommonBaseAB` to a derived class `MR::SeparateA`.
// This is a dynamic downcast, it checks the type before casting. This version will return zero if the target type is wrong.
// This version is acting on mutable pointers.
// The reference to the parameter `object` might be preserved in the return value.
MR_A_API const MR_SeparateA *MR_SeparateA_DynamicDowncastFrom_MR_CommonBaseAB(const MR_CommonBaseAB *object);

// Downcasts an instance of `MR::CommonBaseAB` to a derived class `MR::SeparateA`.
// This is a dynamic downcast, it checks the type before casting. This version will return zero if the target type is wrong.
// The reference to the parameter `object` might be preserved in the return value.
MR_A_API MR_SeparateA *MR_SeparateA_MutableDynamicDowncastFrom_MR_CommonBaseAB(MR_CommonBaseAB *object);
...
MR_A_API const MR_SeparateA *MR_SeparateA_DynamicDowncastFromOrFail_MR_CommonBaseAB(const MR_CommonBaseAB *object);
...
// The reference to the parameter `object` might be preserved in the return value.
// The returned pointer will never be null. It is non-owning, do NOT destroy it.
MR_A_API MR_SeparateA *MR_SeparateA_MutableDynamicDowncastFromOrFail_MR_CommonBaseAB(MR_CommonBaseAB *object);
