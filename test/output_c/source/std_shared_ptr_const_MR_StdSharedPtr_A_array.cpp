...
long MR_C_std_shared_ptr_const_MR_StdSharedPtr_A_array_UseCount(const MR_C_std_shared_ptr_const_MR_StdSharedPtr_A_array *_this)
{
    return (_this ? *(const std::shared_ptr<const MR::StdSharedPtr::A[]> *)(_this) : throw std::runtime_error("Parameter `_this` can not be null.")).use_count();
}
