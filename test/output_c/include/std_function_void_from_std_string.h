...
MR_C_API void MR_C_std_function_void_from_std_string_Destroy(MR_C_std_function_void_from_std_string *_this);

/// Destroys a heap-allocated array of `MR_C_std_function_void_from_std_string`. Does nothing if the pointer is null.
MR_C_API void MR_C_std_function_void_from_std_string_DestroyArray(MR_C_std_function_void_from_std_string *_this);
...
/// Parameter `_this` can not be null.
/// Callback parameter `_1` is never null. It is an instance allocated on the heap! Must call `MR_C_std_string_Destroy()` to free it when you're done using it.
