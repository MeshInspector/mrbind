...
/// Returns an instance allocated on the heap! Must call `MR_C_std_unique_ptr_int_array_Destroy()` to free it when you're done using it.
...
MR_C_API void MR_C_std_unique_ptr_int_array_Destroy(MR_C_std_unique_ptr_int_array *_this);

/// Destroys a heap-allocated array of `MR_C_std_unique_ptr_int_array`. Does nothing if the pointer is null.
MR_C_API void MR_C_std_unique_ptr_int_array_DestroyArray(MR_C_std_unique_ptr_int_array *_this);
...
/// Returns an element from the stored array.
/// Parameter `_this` can not be null.
