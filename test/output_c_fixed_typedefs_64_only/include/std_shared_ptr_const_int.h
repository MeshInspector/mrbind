...
MR_C_API MR_C_std_shared_ptr_const_int *MR_C_std_shared_ptr_const_int_DefaultConstruct(void);

/// Constructs an array of empty (default-constructed) instances, of the specified size. Will never return null.
/// The array must be destroyed using `MR_C_std_shared_ptr_const_int_DestroyArray()`.
/// Use `MR_C_std_shared_ptr_const_int_OffsetMutablePtr()` and `MR_C_std_shared_ptr_const_int_OffsetPtr()` to access the array elements.
MR_C_API MR_C_std_shared_ptr_const_int *MR_C_std_shared_ptr_const_int_DefaultConstructArray(size_t num_elems);

/// Constructs a copy of another instance. The source remains alive.
/// The reference to the parameter `other` might be preserved in the constructed object.
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_C_std_shared_ptr_const_int_Destroy()` to free it when you're done using it.
/// When this function is called, this object will drop any object references it held previously.
MR_C_API MR_C_std_shared_ptr_const_int *MR_C_std_shared_ptr_const_int_ConstructFromAnother(MR_C_PassBy other_pass_by, MR_C_std_shared_ptr_const_int *other);
...
MR_C_API void MR_C_std_shared_ptr_const_int_Destroy(const MR_C_std_shared_ptr_const_int *_this);

/// Destroys a heap-allocated array of `MR_C_std_shared_ptr_const_int`. Does nothing if the pointer is null.
MR_C_API void MR_C_std_shared_ptr_const_int_DestroyArray(const MR_C_std_shared_ptr_const_int *_this);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
/// The reference to the parameter `ptr` might be preserved in the return value.
MR_C_API const MR_C_std_shared_ptr_const_int *MR_C_std_shared_ptr_const_int_OffsetPtr(const MR_C_std_shared_ptr_const_int *ptr, ptrdiff_t i);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
/// The reference to the parameter `ptr` might be preserved in the return value.
MR_C_API MR_C_std_shared_ptr_const_int *MR_C_std_shared_ptr_const_int_OffsetMutablePtr(MR_C_std_shared_ptr_const_int *ptr, ptrdiff_t i);
...
MR_C_API MR_C_std_shared_ptr_const_void *MR_C_std_shared_ptr_const_int_ConvertTo_MR_C_std_shared_ptr_const_void(const MR_C_std_shared_ptr_const_int *_this);
...
MR_C_API void MR_C_std_shared_ptr_const_int_AssignTo_MR_C_std_shared_ptr_const_void(const MR_C_std_shared_ptr_const_int *_this, MR_C_std_shared_ptr_const_void *_target);
