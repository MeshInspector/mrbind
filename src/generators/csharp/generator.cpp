    ...
    void Generator::EmitCFuncLike(OutputFile &file, AnyFuncLikePtr any_func_like, std::string_view csharp_name)
    {
        ...
        try
        {
            ...
            if (is_ctor)
            {
                ...
                {
                    // If the class is backed by a shared pointer, we need to cast null to the correct pointer type.
                    // Our goal here is to construct it without a backing shared pointer at all, instead of constructing a new null shared pointer.
                    file.WriteString(" : this((" + CppToCSharpClassName(ParseNameOrThrow(func_like.ret.cpp_type), true) + "._UnderlyingShared *)null, is_owning: true)");
                }
            }
    ...
    void Generator::EmitMaybeConstCClass(OutputFile &file, const MaybeConstClass &cl)
    {
        try
        {
            ...
            if (shared_ptr_desc)
            {
                ...
                file.WriteSeparatingNewline();
                file.WriteString("internal unsafe " + std::string(unqual_csharp_name) + "(_UnderlyingShared *ptr, bool is_owning) : base(is_owning) {_UnderlyingSharedPtr = ptr;}\n");
                ...
                { // `_LateCreateShared()`, a helper for parsed constructors.
                    // This function is used in generated parsed constructors.
                    // First you put `: this((_UnderlyingShared *)null, true)` in the member init list,
                    //   and then call `_LateCreateShared()` to lazily construct a new shared pointer.

                    file.WriteSeparatingNewline();
                    file.WriteString("private unsafe " + std::string(unqual_csharp_name) + " _LateCreateShared(" + sharedptr_constvoid_underlying_ptr_type + "ownership, " + underlying_ptr_type + "ptr)\n");
                    file.PushScope({}, "{\n", "}\n");
