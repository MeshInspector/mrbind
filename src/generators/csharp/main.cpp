...
int main(int argc, char **argv)
{
    ...
    for (auto &[relative_path, file_desc] : generator.output_files)
    {
        std::string out_path_str = relative_path + ".cs";
        std::filesystem::path out_path = output_dir_fs_path / mrbind::MakePath(out_path_str);
        ...
        if (std::filesystem::exists(out_path))
            throw std::runtime_error("Output file already exists: `" + out_path_str + "`.");
