...
namespace mrbind::C::Modules
{
    struct StdAndTlExpected : DeriveModule<StdAndTlExpected>
    {
        ...
        std::optional<Generator::BindableType> GetBindableType(Generator &generator, const cppdecl::Type &type, const std::string &type_str) override
        {
            ...
            {
                bool is_new = false;
                Generator::OutputFile &file = *generator.GetPublicHelperFileForGroup(generator.FindGroupIndexForType(type), generator.CppdeclToIdentifier(type_with_merged_std_and_tl), &is_new);
