...
namespace mrbind
{
    void ParsedFileToMacros(const ParsedFile &file, std::ostream &out, bool enable_cppdecl, bool pretty_type_spelling)
    {
        out <<
        ...
        if (pretty_type_spelling)
        {
            // See the comments on `default_arguments_need_using_namespace` for what this is.
            out <<
                "\n"
                "#define MR_USING_PRETTY_TYPE_SPELLING\n";
        }
        ...
        { // Dump file contents.
            auto TypeToDefaultSpelling = [&](const Type &type) -> const std::string &
            {
                return pretty_type_spelling ? type.pretty : type.canonical;
            };
            ...
            auto DumpParams = [&](const std::vector<FuncParam> &params, bool indent_twice = true)
            {
                ...
                for (const FuncParam &param : params)
                {
                    if (indent_twice)
                        out << "    ";
                    out << "    ("
                        << "(" << TypeToDefaultSpelling(param.type) << "), "
            ...
            auto DumpLifetimes = [&](const std::vector<FuncParam> &params, const Type *return_type, const Lifetimes &lifetimes, bool is_class_member)
            {
                ...
                for (const LifetimeRelation &lifetime : lifetimes.relations)
                {
                    ...
                    auto TypeIsOk = [&](cppdecl::Type type) -> bool
                    {
                        ...
                        return true; // Do we need to reject anything else?
                    };

                    // Validate the variant, that it makes sense for the type we have.
                    // If this doesn't pass, we ignore this annotation.
                    auto VariantIsOk = [&](const LifetimeRelation::Variant &var) -> bool
                    {
                        if (!enable_cppdecl)
                            return true; // Can't parse the type to analyze it.

                        return std::visit(Overload{
                            [&](const LifetimeRelation::ThisObject &) -> bool
                            {
                                // `this` is a class, so it should always be fine.
                                return true;
                            },
                            [&](const LifetimeRelation::Param &param) -> bool
                            {
                                return TypeIsOk(ParseTypeWithCppdecl(params.at(std::size_t(param.index)).type.canonical));
                            },
                            [&](const LifetimeRelation::ReturnValue &) -> bool
                            {
                                assert(return_type);
                                return return_type && TypeIsOk(ParseTypeWithCppdecl(return_type->canonical));
                            },
                        }, var);
                    };
            ...
            auto lambda = [&](auto &lambda, const Entity &e) -> void
            {
                std::visit(Overload{
                    ...
                    [&](const FuncEntity &e)
                    {
                        BeginMultiplexBlock();
                        out << "MB_FUNC("
                            << "/*returns*/(" << TypeToDefaultSpelling(e.return_type) << "), "
                            << e.name << ", "
                            << e.simple_name << ", "
                            << "(" << e.qual_name << "), "
                            << "(" << e.full_qual_name << "), "
                        ...
                        out << ", ";
                        DumpLifetimes(e.params, &e.return_type, e.lifetimes, false);
                        out << ")\n";
                        EndMultiplexBlock();
                    },
                    [&](const ClassEntity &e)
                    {
                        ...
                        else
                        {
                            out << "\n    /*bases:*/\n";
                            for (const ClassBase &base : e.bases)
                                out << "    ((" << TypeToDefaultSpelling(base.type) << "), " << (base.is_virtual ? "virtual" : "/*not virtual*/") << ")\n";
                            out << "    ,";
                        }
                                ...
                                std::visit(Overload{
                                    [&](const ClassField &field)
                                    {
                                        out
                                            ...
                                            << "(" << TypeToDefaultSpelling(field.type) << "), "
                                            << field.name << ", "
                                            << "(" << field.full_name << "), "
                                    ...
                                    [&](const ClassCtor &ctor)
                                    {
                                        ...
                                        out << ", ";
                                        DumpLifetimes(ctor.params, nullptr, ctor.lifetimes, true); // Note use not passing the return type. The `ReturnValue{}` annotation shouldn't be used for constructors anyway.
                                        out << ")\n";
                                    },
                                    [&](const ClassMethod &method)
                                    {
                                        ...
                                        out
                                            << ", "
                                            << "(" << TypeToDefaultSpelling(method.return_type) << "), "
                                            << method.name << ", "
                                            << method.simple_name << ", "
                                            << "(" << method.full_name << "), "
                                        ...
                                        out << ", ";
                                        DumpLifetimes(method.params, &method.return_type, method.lifetimes, true);
                                        out << ")\n";
                                    },
                                    [&](const ClassConvOp &conv_op)
                                    {
                                        out
                                            ...
                                            << "(" << TypeToDefaultSpelling(conv_op.return_type) << "), "
